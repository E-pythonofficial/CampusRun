...
const AdminDashboard = () => {
  ...
  const handleScheduleInterview = async () => {
    if (!meetLink || !interviewModal) return;
    setActionId(interviewModal.userId);
    try {
      await api.put(`/admin/dispatchers/${interviewModal.userId}/interview`, {
        meetLink,
        scheduledTime: meetTime || undefined,
      });
      setPending(prev => prev.filter(d => d.id !== interviewModal.userId));
